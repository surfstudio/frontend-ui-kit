...
export const Title = (props) => {
    const { children, level = DEFAULT_LEVEL, color, uppercase = true, className, fontWeight } = props,
        rest = __rest(props, ['children', 'level', 'color', 'uppercase', 'className', 'fontWeight']);
    const Tag = `h${level}`;
    return _jsx(
        Tag,
        Object.assign(
            {
                ...
                className: `heading ${className !== null && className !== void 0 ? className : ''} ${uppercase ? 'uppercase' : ''}`,
            },
            rest,
            { children: children },
        ),
