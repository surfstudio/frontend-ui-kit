...
export const Text = (props) => {
    const {
        ...
        rest = __rest(props, ['tag', 'color', 'size', 'type', 'fontWeight', 'uppercase', 'children', 'className']);
    const Tag = tag;
    return _jsx(
        Tag,
        Object.assign(
            {
                ...
                className: `text ${className !== null && className !== void 0 ? className : ''} ${uppercase ? 'uppercase' : ''} ${size === 'small' ? 'small' : ''} ${type === 'caption' ? 'caption' : ''}`,
            },
            rest,
            { children: children },
        ),
